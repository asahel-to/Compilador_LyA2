...
public class VentanaCuadruplosLyA2 extends JFrame {
    private final JTextArea txtTac = new JTextArea();
    private final JTable tabla = new JTable();
    private final DefaultTableModel modelo = new DefaultTableModel(new Object[]{"Op", "Arg1", "Arg2", "Resultado"}, 0) {
        @Override public boolean isCellEditable(int r, int c) { return false; }
    };

    public VentanaCuadruplosLyA2(String tacInicial) {
        setTitle("Cuádruplos-LyA2");
        setSize(900, 600);
    ...
    private void generar() {
        ...
        modelo.setRowCount(0);
        for (ParserTAC.CuadruploTAC c : resultado.getCuadruplos()) {
            modelo.addRow(new Object[]{c.getOp(), c.getArg1(), c.getArg2(), c.getResultado()});
        }

        JFileChooser chooser = new JFileChooser();
        chooser.setDialogTitle("Guardar cuádruplos");
        chooser.setFileFilter(new FileNameExtensionFilter("Archivos de texto (*.txt)", "txt"));
        chooser.setSelectedFile(new File("cuadruplos.txt"));
        if (chooser.showSaveDialog(this) == javax.swing.JFileChooser.APPROVE_OPTION) {
            ...
            try (PrintWriter pw = new PrintWriter(new FileWriter(archivo))) {
                for (ParserTAC.CuadruploTAC c : resultado.getCuadruplos()) {
                    pw.println(c.getOp() + "\t" + c.getArg1() + "\t" + c.getArg2() + "\t" + c.getResultado());
                }
